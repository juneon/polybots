...
### 2026-07-16 (회사) — [회고 삽입 07-20] 수집 + run_grid 체크포인트 + ma 24k 그리드 + P2 스레드 분리 — **미푸시로 집 07-17~19 작업과 중복, 코드·판정은 집 버전으로 대체됨**

> ⚠️ 이 세션은 push 없이 종료 → 집에서 같은 작업(체크포인트·스레드 분리·24k 그리드)을 재수행. 07-20 sync에서 **코드는 집 버전 채택(폐기)**, 로그·결과 파일·리포트만 자산으로 병합. 회사 원본 스냅샷은 로컬 브랜치 `office/20260716-unpushed`에 보존.

- **수집**: threshold sim run `20260716_134816` (13:48~, 구 config 0.85). 로그 병합됨 (events +13,603행 / trades·snapshots +24행)
- **ma 24k 그리드 완주 + D23 미달 → config 변경 없음** (`backtest/results/20260716_grid_ma.{csv,json}` · `reports/20260716_ma_grid_slope_result.html`): H1 재현(top100 전부 slope 사용), 후보 A(cap0.5/ma300/slope−0.005) 다차원 스파이크로 ③ 미달, H2·H3 기각 — **집 07-18 완주(sim 37)·07-19 재판정(sim 60)과 동일 결론에 도달한 독립 재현**으로만 의미. 정본은 집 결과
- **run_grid 증분 체크포인트 + P2 SELL 스윕 스레드 분리 구현** — 집 07-17(a795b9d)·07-18(b303944)에서 재구현되어 이쪽이 정본. 회사 구현과의 설계 차이 1건은 P2 라이브 실측 시 검토 항목으로 아래 07-20 sync에 기록
- 세션 정리: `reports/20260716_session_wrap_result.html`

### 2026-07-17 (집) — 수집 재개 + ma 24k 그리드 집 재실행 → 새벽 강제종료, 07-18 복구 (체크포인트 첫 실전 검증)

> ⚠️ 배경: 07-16 회사 그리드 완주분이 **미푸시 상태로 이틀째** (원격 main은 집 커밋 1004af1이 마지막) → 집에서 재실행하는 쪽으로 선회한 세션. 회사 push가 올라오면 WORKLOG·run_grid.py merge 충돌 예상 — 이 파일 기준으로 정리할 것.

- **수집**: threshold sim run `20260717_183301` (18:33~) — 12 slugs 관측 중 **07-18 00:59 강제종료** (heartbeat 정지). 포지션 없이 끊김 (`position: null`, cash −15.21) — 이월 정산 이슈 없음
- **그리드**: parquet 재생성(18:52) 직후 24,192콤보 풀 그리드 시작(18:56, workers 12) — **07-18 01:01 강제종료 시점 19,374/24,192 (80.1%)가 `results/20260717_185646_grid_ma.csv`에 저장됨**. a795b9d 증분 체크포인트가 첫 실전에서 작동 (07-15 "56% 유실"과 대비). 마지막 행 손상 없음
- **07-18 01:14 복구** (세션 기록 없이 종료됐던 것을 로그·heartbeat·CSV로 역추적):
  - 그리드 재개: 같은 `--out`으로 재실행 → 체크포인트 인식 "19374 already → 4818 to run", rate ~2.6/s ETA ~30분. **주의: data_prep 재실행 금지** — events.csv가 그리드 시작 후에도 수집돼, parquet 재생성 시 sim 소스가 앞 80%와 불일치하게 됨. 이번 그리드는 07-17 18:52 parquet 기준으로 완주
  - 수집 재시작: run `20260718_011522` (분리 프로세스, stdout은 logs/ctl로) — 계좌 이어짐 확인
- **완주 후 할 일**: ① D23 3중 기준 판정 + `reports/20260715_ma_grid_slope_result.html` (민감도 차트, 가설 H1~H3은 plan에 등록됨) ② 그리드 결과 + 로그 스냅샷 커밋/push ③ 회사 미푸시 결과와의 정합은 push 후 대조(같은 축이면 집 결과로 대체 가능)

### 2026-07-18 (집, 새벽) — ma 24k 그리드 완주 + D23 판정: **config 변경 없음** (③ 미달), slope 축은 검증

완주 02시경 (완전 214 slugs = janfeb 161 / mar03 16 / sim 37, haircut 0.01/p_fail 0.2/seed 42). 상세·차트는 `reports/20260715_ma_grid_slope_result.html`, 원 데이터 `backtest/results/20260717_185646_grid_ma.{csv,json}`.

**가설 판정 (plan 사전 등록분)**:
- **H1 확정 — slope 필터는 실엔진에서도 유효**: 매치드 페어 6,048쌍에서 slope 0/−0.005/−0.01 전부 train·val 평균 동시 개선(−0.01이 최대: Δtrain +14.4, Δval +5.4, 56% 둘 다 개선). train 상위 100개 중 slope 미적용 **0개**. 동시 양수 조합 533개(2.2%, 07-14 그리드 0.9%의 배 이상)의 83%가 slope ≤ 0
- **H2 확인 — cap0.6~0.7/ma600은 slope와 중복 효과**: 그 영역에선 slope 추가가 무효과(val 오히려 소폭 악화). 영역 최선(val +24.19)도 train −17.98로 ① 미달 — 07-13 "전 구간 생존형" 후보 탈락
- **H3 기각 — 07-14 후보 영역(cap0.45/ma200/tp0.99) 소멸**: 동시 양수 0개. sim 26→37 표본 확대만으로 사라짐 (소표본 후보 영역의 취약성 실례 — 재판정에 sim 60을 요구하는 이유)

**D23**: ① 충족(후보 A cap0.5/ma300/tc0/tp없음/cd0/slope−0.005 = train 44.71 전체 4위) ② 형식 충족(A val +5.75, B=현 config+slope만 val +6.48) — 단 **mar03은 상위 전 영역 음수**(val 양수는 전적으로 sim 기여) ③ **미달**: tp·ban 축은 plateau지만 **ma_len 240/480 절벽(−35/−16) + cooldown 30 train 붕괴(−14)**, cap·slope 이웃 val 부호 반전 — ma300은 능선이지 평원이 아님. **→ configs/ma.json 변경 없음**, 후보 영역(cap0.5~0.6/ma300/tc0~2/cd0/slope−0.005~−0.01, 대표 A·B)을 **차기 재판정(완전 sim 60, 현 37)의 1순위 비교 대상**으로 등록

**부가 발견**: 구 config는 slope 한 칸만 더해도 score 7.78(397위)/val −17.61 → 37.25/+6.48로 도약 (B) · haircut 민감도 — 상위 전 조합이 0.03에서 음전, 0.02 여유는 A(+19.62)가 최대 · 증분 체크포인트 첫 실전 성공(80.1% 저장분 이어 29분 완주)

**다음**: 수집 계속(완전 sim 37/60) → 60 도달 시 threshold(t180/c20~30·stable 계열)와 ma(A·B 영역) 동시 재판정. MA sim 재가동은 여전히 P3 recorder 분리와 연계 별도 결정

### 2026-07-18 (집, 새벽 이어서) — 재판정 준비: sweep에 `enter_stable_sec` 축 추가 (수집 대기 중 작업)

- `backtest/sweep_threshold.py`: **`ENTER_STABLE` {0,15,30,60} 축 추가** + `STOP_CONFIRM`을 사전 등록 계획대로 {0,20,30}으로 정리(07-14 판정: 5/10은 노이즈) → 1,728콤보. 현 config 매칭·출력에 stable 반영. sim 60 도달 즉시 재판정 스윕 실행 가능 상태
...
### 2026-07-18 (집, 오후) — 수집 목표 달성: **완전 60/60** ✅ (연장 목표 완료)

- 수집 재개: run `20260718_125732` (12:57~16:12, 분리 프로세스) — 45→**60/60** 도달 즉시 stop-file 그레이스풀 종료 (`stop_reason: stop_requested`, 포지션 없음, 이월 정산 이슈 없음). 관측 총 90 slugs (threshold 90 / ma 36, 전략 간 중복 제외)
- 이번 run 성적: 17 slugs 관측, 왕복 10회 순 −$0.20 수준 (개별: +1.00/−0.10/+1.30/−1.30/+1.30/−1.20/−1.40/−2.00/+1.20/−2.50/+1.30/+1.00/+0.50 중 스탑 손실 몇 건 포함). 새 config(0.85/0.12/tp0.98) 특성대로 스탑아웃이 손익 결정 요인
- events 일자 로테이션 첫 실전: 이번 run부터 `logs/events_20260718.csv`에 기록 (git 추적 시작)
- 커밋: `ba10749` logs snapshot (60/60), push 완료. **이 시점 원격 main이 재판정 기준 데이터의 정본**

**다음 스텝 (재개 시 여기부터 — 순서대로)**:
1. `python -m backtest.data_prep` 재빌드 — 새 sim 소스(완전 60) 반영. 파일별 캐시 덕에 오늘 파일만 재파싱
2. **threshold 재판정**: `backtest/sweep_threshold.py` 풀 스윕 1,728콤보 (enter_stable_sec {0,15,30,60} 축 포함, 07-18 새벽 준비 완료) → D23 3중 기준 판정. 관심 영역: t180/c20~30 + stable 조합 (07-14 가설)
3. **ma 재판정**: 후보 A(cap0.5/ma300/tc0/tp없음/cd0/slope−0.005)·B(현 config+slope−0.005) 영역이 sim 60에서도 생존하는지 — 07-18 새벽 그리드(24k)의 1순위 비교 대상으로 등록된 것. 필요시 해당 영역만 부분 그리드
4. 판정 결과에 따라 config 교체 여부 결정 + reports/에 result HTML
- 참고: 회사 미푸시 07-16 그리드 결과는 집 07-18 완주분이 정본이 되어 사실상 대체됨 — 회사 push 시 WORKLOG/run_grid.py 충돌은 집 버전 기준으로 정리

### 2026-07-19 (집) — sim 60 재판정 완료: **threshold config 교체** (0.90/c30/s60), ma 변경 없음

07-18 "다음 스텝" 1~4 실행. 데이터: 완전 237 slugs (janfeb 161 / mar03 16 / **sim 60**), haircut 0.01/p_fail 0.2/seed 42. 상세는 `reports/20260719_sim60_reverdict_result.html`, 원 데이터 `backtest/results/20260719_205030_sweep_threshold.{csv,json}`(1,728콤보) + `20260719_205628_grid_ma_partial60.{csv,json}`(후보 영역 576콤보).

**threshold — config 교체 단행**:
- 현행(0.85/0.12/tp0.98/t450/c0/s0)은 sim 60에서 **val −6.36 (sim −5.41), rank 59/1728** — 07-13 교체 근거였던 sim 33이 60으로 늘자 음전 확정. 07-18 run 실측(순 −$0.20)과 정합
- score 1위 영역(t180/c30/s30, 07-14 가설 적중)은 **나이프엣지로 기각**: 전 축 이웃 절벽(t300 → −16.25, c0 → −9.59, s15 → −12.24, 0.80 → −15.51, cap0.95 → −7.44) + sim −0.60 (val 양수가 전부 mar03 기여)
- **채택: 0.90/0.12/tp0.98/cap0.9/t450/confirm30/stable60** — 3소스 동시 양수 21/1728(1.2%) 중 score 1위(6.35, val +2.65). 공통 시그니처는 `0.90 + confirm30 + stable>0`. stop_drop 0.06~0.12 전 구간·t_enter 180~450 전 구간 부호 유지, stable/confirm은 on/off 절벽(파라미터 미세조정이 아니라 필터 기능 효과 — 07-14 "스파이크 직후 진입 78% 무엣지"와 정합). tp0.99는 sim 반전이라 0.98 유지
- **캐비앗**: 46 fills(W19/L4) 소표본, effect size 작음 — "덜 잃는" 보수 전환이지 엣지 입증 아님. **새 config 수집 관찰이 본 검증** (차기 재판정 트리거 제안: 신규 완전 slug +30)
- config diff: enter 0.85→0.90 / stop_confirm 0→30 / enter_stable 0→60 (나머지 유지). 테스트 93 통과. 진입 빈도 ~1/8로 감소가 정상

**ma — 변경 없음 유지** (07-18 판정이 sim 60에서 재현):
- 후보 A val +5.75→+7.15, B +6.48→**+12.58** — 영역 생존·개선(①② 강화, 07-14 후보처럼 소멸하지 않음), 영역 24콤보 중 val>0 19개
- 그러나 ③ 여전히 미달: ma240 절벽(A: train −22/val −17), cap 이웃 반전(B: 0.45 → −5.5, 0.6 → −0.1), slope 이웃 반전(A: −0.01 → −1.15), cd30 train 붕괴(−14)
- **결정적: 후보 영역 24/24 전부 mar03 음수** — val 양수 100% sim 기여, 유일한 라이브 아웃오브샘플 전패 불변
- 부가: cap0.6/tc2 서브영역이 val +17~22 최상위(6/6 양수, train 20~33 평범) — 차기 관찰 후보로만 등록

**다음 스텝**: ① 새 threshold config로 sim 수집 재개 (관찰: stable60/confirm30이 실시간에서 의도대로 거르는지, 스탑아웃 감소 여부) ② 신규 완전 +30 누적 시 threshold 재검증 ③ P2 라이브 실측·P3 recorder 분리는 별도 트랙

### 2026-07-20 (회사) — 회사↔집 sync: 07-16 미푸시 세션 병합 정리
...
### 2026-07-20 (회사, 이어서) — threshold 알파 논의 + flip 스크리닝: **완전 뒤집기 기각** (부정적 결과)

**배경**: 사용자 테제 = "slug 내 모멘텀 플레이 + 좋은 손익비인데 왜 알파가 없나". 캘리브레이션 프레임(07-14)으로 답 — 호가에 보이는 모멘텀은 이미 가격(스파이크 진입 78% 무엣지), 손익비는 캘리브레이션된 바이너리에서 설계 변수가 아님(손절 시점 bid = 회복 확률). 이어서 "완전히 뒤집으면(favorite 대신 underdog 매수)?" 가설을 간이 스크리닝.

**flip 스크리닝** (`backtest/adhoc/20260720_flip_screen.py`, 완전 slug, 진입 순간 favorite ask ≥ 0.85/0.90 × stable 0/60s, 만기 보유, ask 스프레드 + 헤어컷 0.01 반영):
- **방향은 레짐 스토리와 정합**: flip EV(토큰당)는 sim(7월)에서만 양수 +$0.012~0.017, janfeb −0.024~−0.038 / mar03 −0.061~−0.091 — 7월 favorite 과대평가의 거울상 그대로
- **그러나 기각 사유 3개**: ① sim 엣지가 1SE(±0.035~0.040)에도 못 미침 (t≈0.4, n 51~72 — Q1의 "2se 유의 없음"과 동일) ② 유일한 라이브 oos(mar03) 전패 ③ **양방향 왕복 마찰 ≈ $0.06/토큰** (fav_EV + flip_EV ≈ −0.06으로 실측) — favorite가 7.6센트 과대평가된 구간에서도 뒤집으면 1.7센트만 남음. 미스프라이싱의 대부분을 스프레드가 흡수
- **함의**: 완전 뒤집기는 "7월 레짐 지속" 베팅일 뿐 구조적 엣지가 아님 (레짐 게이트는 Q3에서 이미 기각). 부산물로 마찰 총량이 실측됨 — 레버 ②(비용, maker 진입)와 ①(Binance lead-lag = 호가가 아직 모르는 모멘텀)의 우선순위 재확인
- 다음 알파 후보로 **Binance lead-lag 스터디** 설계 논의 (기초자산 모멘텀 vs Polymarket 호가 지연 — P4 어댑터 씨앗과 연결)
